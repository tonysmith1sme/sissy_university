		...
		class et {
			...
			static LoadData(t) {
				...
				n.append($(`<img class="img-fluid viewObject-img hoverable" src="${u}.webp" onerror="this.onerror=null; this.src='${u}.jpg'" alt="${s.name}">`).on("click", function() {
					dt.RefreshView()
				})).append($("<h2>").text(s.name)).append($("<h5>").append(o)).append($("<p>").text(s.description)), n.append($("<hr>")).append($("<h5>专业毕业考:</h5>")).append($("<p>").append($("<b>").text("专业考试 1: ")).append($("<span>").text(s.exam1))).append($("<p>").append($("<b>").text("专业考试 2: ")).append($("<span>").text(s.exam2))).append($("<p>").append($("<b>").text("专业考试 3: ")).append($("<span>").text(s.exam3))), t.unlocked && n.append($("<hr>")).append($('<button class="btn btn-success btn-margin">专业考试  1</button>').on("click", function() {
					let t = {
						entityId: s.id,
						entityType: "major",
						entityOption: "exam1"
